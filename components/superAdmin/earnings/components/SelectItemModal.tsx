// src/app/super-admin/earnings/components/SelectItemModal.tsx
"use client";

import React, { useState } from 'react';
import { earningsData } from './data';

export const SelectItemModal = ({ category, onShowResults, onCancel }) => {
    const [selectedLocation, setSelectedLocation] = useState('');
    const [selectedItem, setSelectedItem] = useState('');
    const dataForCategory = earningsData[category] || [];
    ...
    const handleShowResults = () => {
        if (selectedLocation && selectedItem) {
            ...
            alert(`Please select a location and a ${category.toLowerCase()}.`);
        }
    };
    ...
    return (
        <div className="absolute inset-0 flex justify-start items-start p-2">
            <div className="bg-[#FACB9F] rounded-lg p-3 md:p-6 w-3/4 md:w-4/6 max-w-2xl shadow-lg">
                <h3 className="font-bold text-red-600 mb-4 text-[9px] md:text-[15px] lg:text-[15px] xl:text-[15px] 2xl:text-[15px]">SELECT A {category} TO SEE PAYOUTS</h3>
                <div className="grid grid-cols-2 md:gap-6">
                    ...
                    <div>
                        <h4 className="font-bold text-black mb-2 text-[9px] md:text-[15px] lg:text-[15px] xl:text-[15px] 2xl:text-[15px]">{category}</h4>
                        <div className="space-y-2">
